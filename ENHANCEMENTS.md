...
| **2. Foundation — ✅ done** | Login (`app/auth.py`) and per-user data folders. **Portfolio snapshots** (daily equity curve), **decisions** (Bought/Passed → "your actual results"), and an **import audit log** with one-click undo. **Postgres support**: `db/connection.py` abstracts the backend — SQLite stays the zero-config default, and setting `DATABASE_URL` switches the whole app to Postgres. `scripts/migrate_to_postgres.py` copies existing SQLite data over (ids preserved). Tested end-to-end on both backends. |
| **3. Community MVP — ✅ done** | New **Community** page (`db/community.py` + dashboard) built inside Streamlit: opt-in **profiles** (bio/avatar, private by default), **follow** + activity **feed**, a **verified-returns leaderboard** (ranked from members' own timestamped buy decisions — not self-reported), per-ticker **discussion threads**, publish/clone **shared watchlists**, and **moderation** (block/report, post rate-limit + length caps, HTML-escaped user content). Privacy-first: only percentages and picks are ever shared, never dollar amounts. Dual-backend (SQLite/Postgres) and covered by unit + AppTest interaction tests. |
| **4. Scale-out — ✅ MVP done** | **FastAPI backend** (`api/`) wraps the existing engine (`db.*`, `data.loader` unchanged) with stateless bearer-token auth and REST endpoints for auth, profiles, the full community surface, and personal data — dual-backend, tested with TestClient on SQLite and Postgres. **Next.js frontend** (`web/`, App Router + TypeScript) consumes it: login/signup, post composer, feed, and the verified-return leaderboard — builds and type-checks clean, verified serving live against the running API. Streamlit remains the internal analysis dashboard. Remaining UI (threads, shared-list browsing, profile editor) is already exposed by the API and is a straight frontend addition. |
